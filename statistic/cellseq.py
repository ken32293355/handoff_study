"""
This code check whether two cellphone connect to the same base station
at the same time
"""
...
def compare(file1, file2):
    ...
    while i < len(df1) and j < len(df2):
        if df1.loc[i, "Date"] < df1.loc[j, "Date"]:
            i += 1
        elif df1.loc[i, "Date"] < df1.loc[j, "Date"]:
            j += 1
        else:
            if df1.loc[i, "PCI"] == df2.loc[j, "PCI"] and df1.loc[i, "CID"] == df2.loc[j, "CID"]:
                same_count += 1
            j += 1
            i += 1
        
    print(same_count, same_count / len(df1))
...
if __name__ == "__main__":
    ...
    datasetpath = r"C:\Users\USER\Desktop\data"
    file1 = os.path.join(datasetpath, "ss1", "round1.csv")
    file2 = os.path.join(datasetpath, "ss2_A42", "round1.csv")
    compare(file1=file1, file2=file2)


    datasetpath = r"C:\Users\USER\Desktop\data"
    file1 = os.path.join(datasetpath, "ss1", "round2.csv")
    file2 = os.path.join(datasetpath, "ss2_A42", "round2.csv")
    compare(file1=file1, file2=file2)

    datasetpath = r"C:\Users\USER\Desktop\data"
    file1 = os.path.join(datasetpath, "ss1", "round3.csv")
    file2 = os.path.join(datasetpath, "ss2_A42", "round3.csv")
    compare(file1=file1, file2=file2)

    datasetpath = r"C:\Users\USER\Desktop\data"
    file1 = os.path.join(datasetpath, "ss1", "round4.csv")
    file2 = os.path.join(datasetpath, "ss2_A42", "round4.csv")
    compare(file1=file1, file2=file2)

    datasetpath = r"C:\Users\USER\Desktop\data"
    file1 = os.path.join(datasetpath, "ss1", "round5.csv")
    file2 = os.path.join(datasetpath, "ss2_A42", "round5.csv")
    compare(file1=file1, file2=file2)
