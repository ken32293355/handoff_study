...
def connection():
    ...
    print("wait for udp1 say hi...")
    indata, udp_addr1 = s_udp.recvfrom(1024)
    print('udp1 Connected by', udp_addr1)
    print("wait for udp2 say hi...")
    indata, udp_addr2 = s_udp.recvfrom(1024)
    print('udp2 Connected by', udp_addr2)
    print('server start at: %s:%s' % (HOST, PORT))
    return s_tcp, s_udp, conn, tcp_addr, udp_addr1, udp_addr2  
...
def transmision(s_udp, conn, udp_addr1, udp_addr2):
    ...
    while time.time() - start_time < total_time and not thread_stop:
        ...
        s_udp.sendto(outdata, udp_addr1)
        s_udp.sendto(outdata, udp_addr2)
        i += 1
...
while not exit_program:
    ...
    time.sleep(2)
    s_tcp, s_udp, conn, tcp_addr, udp_addr1, udp_addr2= connection()
    thread_stop = False
    t = threading.Thread(target = transmision, args = (s_udp, conn, udp_addr1, udp_addr2))
