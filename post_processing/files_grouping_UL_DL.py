...
for dirname in all_file_dir:
    ...
    for UL_pcap_file in UL_pcap_files:
        ...
        pcap = dpkt.pcap.Reader(f)
        try:
            _, latency = get_loss_latency_UL(pcap)
    ...
    for cell_phone_pcap_file in cell_phone_pcap_files:
        ...
        pcap = dpkt.pcap.Reader(f)
        try:
            _, latency = get_loss_latency_DL(pcap)
            store_interval.append(I.closed(latency[0][0],latency[0][-1]))    
        except:
            store_interval.append(I.empty())
            continue
    ...
    for each_group, each_group_time in zip(grouping, interval):
        #print(each_group)
        if each_group[0] != "None" and each_group[1] != "None" and each_group[2] != "None" and each_group[3] != "None":
